...
const double xMinCor(842);
const double yMinCor(390);
const double xMaxCor(1300);
const double yMaxcor(775);
...
bool pointcloud_simplicate_areabased(Scene &scene, vector<Matrix3x4d> &pMatVec, const char*imagePath, double removed_percentage, int lineWidth)
{
	typedef CGAL::Exact_predicates_inexact_constructions_kernel Kernel;
	typedef Kernel::Point_3 Point;
	typedef Kernel::Vector_3 Vector;
	...
	for (int pointIndex = 0; pointIndex < scene.pointcloud.points.size(); pointIndex++)
	{
		...
		if (indexSet.find(pointIndex) == indexSet.end() ||
			pointMVS.x<xMinCor ||
			pointMVS.x>xMaxCor ||
			pointMVS.y<yMinCor ||
			pointMVS.y>yMaxcor)
		{
			pointIndexVec.push_back(pointIndex);
		}
	}

	const int indexSize(pointIndexVec.size());
	for (int i = indexSize - 1; i > -1; i--)
	{
		scene.pointcloud.RemovePoint(pointIndexVec.at(i));
	}
	scene.pointcloud.Save("simplicated.ply");
	scene.Save("simplicated.mvs");
	return true;
...
int main(int argc, char** argv)
{
	...
	pointcloud_simplicate_areabased(scene, pMatVec, argv[3], atof(argv[4]), atoi(argv[5]));
